...
class DatasetDownstream(Dataset):
    def __init__(self, data_path, df=None, ptsz=32):
...
if __name__ == "__main__":
    path = "C:\\Users\\RedmiBook\\HUST\\Documents\\Studying\\Phase_2_VDT\\code\\Project_code\\"
    df = pd.read_csv(path + "data\\cedar.csv")
    datassl = DatasetSSL(data_path=path + "data\\cedar\\", df=df)
    datads = DatasetDownstream(data_path=path + "data\\cedar\\", df=df)
    dlssl = DataLoader(dataset=datassl, batch_size=4)
    dlds = DataLoader(dataset=datads, batch_size=4)
    print(datassl[0][0].shape)
    print(datassl[0][1].shape)
    for x, y in dlssl:
        print(x.shape)
        print(y.shape)
        break
    print(datads[0][0].shape)
    print(datads[0][1], datads[0][2])
    for x, y, z in dlds:
        print(x.shape)
        print(y, z)
        break